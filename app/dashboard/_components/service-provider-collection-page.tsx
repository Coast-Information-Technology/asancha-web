...
// File: app/dashboard/_components/service-provider-collection-page.tsx

/**
 * Asancha Service Provider Collection Page
 *
 * Purpose:
 * Provides reusable safe rendering for bookings, documents, verification,
 * conversations, and payments.
 *
 * Security notes:
 * - Backend responses must be scoped to the active service-provider profile.
 * - Raw booking, payment, document, verification, provider, and conversation
 *   records must be mapped into this safe contract before rendering.
 */
...
import type {
    ServiceProviderCollectionItem,
    ServiceProviderCollectionResponse,
} from "../_types/service-provider-dashboard.types";

export interface ServiceProviderCollectionPageProps {
    config: ServiceProviderCollectionPageConfig;
}
...
export function ServiceProviderCollectionPage({
    config,
}: ServiceProviderCollectionPageProps) {
    const [collection, setCollection] =
        useState<ServiceProviderCollectionResponse | null>(
            null,
        );
    ...
    const loadCollection =
        useCallback(async (): Promise<void> => {
            ...
            try {
                const result =
                    await authApiGet<ServiceProviderCollectionResponse>(
                        config.endpoint,
                    );
                ...
                setErrorMessage(
                    "We could not load this service-provider workspace.",
                );
            } finally {
                setIsLoading(false);
            }
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {Array.from({
                        ...
                        (
                            ...
                            <div
                                key={index}
                                className="h-60 animate-pulse rounded-[var(--asancha-radius-lg)] bg-[var(--muted)]"
                            />
                        ),
                    )}
                </div>
