...
// File: app/dashboard/_components/property-agent-collection-page.tsx

/**
 * Asancha Property Agent Collection Page
 *
 * Purpose:
 * Provides reusable safe list rendering for listings, authority documents,
 * documents, verification, bookings, conversations, and payments.
 *
 * Security notes:
 * - Backend responses must be active-profile scoped.
 * - Raw provider data, internal notes, hidden assignments, and private document
 *   URLs must not enter this component.
 */
...
import type {
    PropertyAgentCollectionItem,
    PropertyAgentCollectionResponse,
} from "../_types/property-agent-dashboard.types";

export interface PropertyAgentCollectionPageProps {
    config: PropertyAgentCollectionPageConfig;
}
...
export function PropertyAgentCollectionPage({
    config,
}: PropertyAgentCollectionPageProps) {
    const [
        collection,
        setCollection,
    ] = useState<PropertyAgentCollectionResponse | null>(
        null,
    );
    ...
    const loadCollection =
        useCallback(async (): Promise<void> => {
            ...
            try {
                const result =
                    await authApiGet<PropertyAgentCollectionResponse>(
                        config.endpoint,
                    );
                ...
                setErrorMessage(
                    "We could not load this property-agent workspace.",
                );
            } finally {
                setIsLoading(false);
            }
